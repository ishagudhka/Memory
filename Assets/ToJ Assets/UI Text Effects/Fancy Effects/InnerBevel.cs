...
public class InnerBevel : BaseMeshEffect, IMaterialModifier
{
	...
	public override void ModifyMesh (VertexHelper vh)
	{
		...
		for (int i = 0; i < count; i += 6)
		{
			...
			Vector2 right = (m_Verts[i + 1].uv0 - m_Verts[i].uv0).normalized;
			Vector2 up = (m_Verts[i + 1].uv0 - m_Verts[i + 2].uv0).normalized;
			...
			for (int j = 1; j < 6; j++)
			{
				uiVertex = m_Verts[i + j];
				uiVertex.tangent = m_Verts[i].tangent;
				if (uiVertex.uv1 == Vector2.zero)
				{
					uiVertex.uv1 = new Vector2(1, 1);
				}
				m_Verts[i + j] = uiVertex;
			}
		}
